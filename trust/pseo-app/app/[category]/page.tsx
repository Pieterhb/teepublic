...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `Shop the best ${category.title.toLowerCase()} created by independent artists. Find the perfect gift or addition to your wardrobe today.`,
    alternates: {
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-slate-900 text-white py-24 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10 text-center">
          <p className="text-blue-400 font-semibold tracking-widest uppercase mb-4 text-sm">Curated Collection</p>
            ...
            <div className="flex flex-wrap justify-center gap-4">
              {relatedCategories.map(cat => (
                <Link 
                  key={cat.slug} 
                  href={`/${cat.slug}`}
                  className="px-6 py-3 bg-white border-2 border-slate-100 hover:border-blue-600 text-slate-700 hover:text-blue-700 rounded-xl font-semibold shadow-sm hover:shadow-md transition-all duration-300"
                >
                  {cat.title}
                </Link>
              ))}
